...
### [Bind mounts - `-v HOSTPATH:CONTAINERPATH`](https://docs.docker.com/engine/storage/bind-mounts)
When to use: 
...
### [Docker copy - `docker copy ](https://docs.docker.com/reference/cli/docker/container/cp/)
...
Remember a container can still have vulnerabilities; it is still running software that has security problems / exploits.  The neat bit is only that those vulnerabilities are on software running on the container, not on the host (isolation).  

There are tools that can scan your software and container and check for vulnerabilities.  Trivvy is one of many, but since Trivvy is open source it is the one I will favor.

> Trivy (pronunciation) is a comprehensive and versatile security scanner. Trivy has scanners that look for security issues, and targets where it can find those issues. - [Trivvy](https://github.com/aquasecurity/trivy)
